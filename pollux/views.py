...
from .map_desc import DATA_ORIGIN
...
class DataDetails(View):
    ...
    def get(self, request, filename):
        if filename not in self.no_way_files:
            try:
                return self.print_db_to_json(
                    self.db_name_to_cls[filename], **request.GET
                )
            except KeyError:
    ...
    def print_file_to_json(self, filename, **kwargs):
        ...
        for directory in self.db_dirs:
            try:
                with open(
                    os.path.join(BASE_DIR, "pollux", directory, filename), "r"
                ) as file:
                    geo = Geojson()
                    if "filters" in kwargs:
                        filters = json.loads(kwargs["filters"][0])
                        filters = [float(pos) for pos in filters["position__within"]]
                        filters = [filters[1], filters[0], filters[3], filters[2]]
    ...
    def print_db_to_json(self, cls, **kwargs):
        ...
        if kwargs["layer_id"][0] != "-1" and kwargs["map_id"][0] != "-1":
            ...
            filters_updated.update(
                CONFIGS[map_id]["layers"][layer_id].get("filters", {})
            )
        ...
        for k, v in filters_updated.items():
            ...
            try:
                ...
                # provisoire
                elif k == "position__dwithin" and filters_updated.get(
                    "position__within"
                ):
        ...
        for lamp, geo_lamp in zip(queryset, response["features"]):
            geo_lamp["properties"]["max_range_day"] = lamp.max_range(
                nb_lux=3, time="day"
            )
            geo_lamp["properties"]["max_range_night"] = lamp.max_range(
                nb_lux=3, time="night"
            )
            colour_impact = (
                60 + 20 * max(0, min(4000, lamp.colour - 2000)) / 4000
            ) / 80
            geo_lamp["properties"]["impact_tree_on_ground"] = (
                (
                    lamp.lux_with_distance(lamp.height, "day")
                    + lamp.lux_with_distance(lamp.height, "night")
                )
                / 2
                * colour_impact
            )
...
class MentionsLegales(MixinSecondaryPages):
    ...
    @property
    def extra_content(self) -> dict:
        return {**super().extra_content, **{"data_origin": DATA_ORIGIN}}
